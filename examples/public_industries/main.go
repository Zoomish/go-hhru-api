...
func main() {
	...
	if defaultUA == "" {
		defaultUA = "go-hhru-api/examples/public_industries (mailto:noreply@example.com)"
	}
	...
	host := public.GetIndustriesParamsHostHhRu
	resp, err := c.Public.GetIndustriesWithResponse(context.Background(), &public.GetIndustriesParams{
		HHUserAgent: *ua,
		Host:        &host,
	})
	if err != nil {
	...
	for _, b := range *resp.JSON200 {
		fmt.Println(b.Id, b.Name, len(b.Industries))
	}
}
